...
import { FloatingShapes } from "@/components/ui/animated-backgrounds"
...
export function Marketplace() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true)
      try {
        const allProducts = await supabaseDb.getApprovedProducts()

        const mappedProducts = await Promise.all(allProducts.map(async p => {
          const creatorData = await supabaseDb.getUser(p.creator_id)
  ...
  return (
    <div className="min-h-screen pt-20 pb-8 bg-background relative overflow-hidden">
      ...
      <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_center_left,rgba(16,185,129,0.03),transparent_50%)]" />
      ...
      {/* Enhanced floating orbs with glow-breathe */}
      <div className="absolute top-40 right-[10%] w-72 h-72 bg-gradient-to-br from-ambient-500/15 to-transparent rounded-full blur-3xl animate-float-orb animate-glow-breathe" />
      <div className="absolute bottom-40 left-[5%] w-96 h-96 bg-gradient-to-br from-purple-500/10 to-transparent rounded-full blur-3xl animate-float-orb-reverse" style={{ animationDelay: '-8s' }} />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-gradient-to-br from-cyan-500/5 to-transparent rounded-full blur-3xl animate-morph" />
