...
export function Sidebar({ currentPage }: SidebarProps) {
    ...
    return (
        <aside className="fixed left-0 top-0 h-screen w-64 bg-background backdrop-blur-3xl border-r border-border/50 flex flex-col z-50 shadow-2xl animate-border-glow">
            {/* Enhanced Ambient Nebula Background */}
            <div className="absolute inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-0 left-0 w-full h-48 bg-gradient-to-b from-ambient-500/8 to-transparent" />
                <div className="absolute bottom-0 left-0 w-full h-48 bg-gradient-to-t from-purple-500/8 to-transparent" />
                <div className="absolute inset-0 bg-[linear-gradient(to_bottom,#ffffff01_1px,transparent_1px)] bg-[size:100%_40px] opacity-30" />

                {/* Floating particles */}
                <div className="absolute top-20 left-8 w-1 h-1 bg-ambient-400 rounded-full animate-pulse-glow" />
                <div className="absolute top-40 right-12 w-1.5 h-1.5 bg-purple-400 rounded-full animate-pulse-glow" style={{ animationDelay: '1s' }} />
                <div className="absolute bottom-32 left-16 w-1 h-1 bg-cyan-400 rounded-full animate-pulse-glow" style={{ animationDelay: '2s' }} />
                <div className="absolute top-1/2 right-8 w-0.5 h-0.5 bg-ambient-300 rounded-full animate-pulse-glow" style={{ animationDelay: '0.5s' }} />

                {/* Floating orb */}
                <div className="absolute top-1/3 left-1/2 -translate-x-1/2 w-32 h-32 bg-gradient-to-br from-ambient-500/5 to-transparent rounded-full blur-2xl animate-float-orb" />
            </div>
            ...
            <div className="p-4 mt-auto border-t border-border">
                ...
                <div className="flex items-center space-x-3 mb-4 px-3 py-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-all duration-300 cursor-pointer group border border-border">
                    ...
                    <div className="flex flex-col min-w-0 flex-1">
                        ...
                        <p className="text-[10px] text-muted-foreground truncate capitalize flex items-center gap-1.5 font-mono">
                            ...
                            {profile?.role === "creator" ? "Seller" : profile?.role === "admin" ? "Admin" : "User"}
                        </p>
                    </div>
                </div>
                ...
                <div className="space-y-2 pt-2 relative z-[60]">
                    <Link href="/dashboard/account" className="block">
                        ...
                        >
                            <Settings className="w-4 h-4 mr-3 group-hover:rotate-90 transition-transform duration-500" style={{ color: '#e2e8f0' }} />
                            Settings
                        </button>
                    </Link>
                    <button
                    ...
                    >
                        <LogOut className="w-4 h-4 mr-3 group-hover:-translate-x-1 transition-transform" style={{ color: '#fecaca' }} />
                        Sign Out
                    </button>
                </div>
            </div>
