...
export function ProductPage() {
  ...
  // Fetch product from Supabase based on URL params
  useEffect(() => {
    const fetchProduct = async () => {
      ...
      try {
        const fetchedProduct = await supabaseDb.getProduct(productId)
        if (fetchedProduct) {
          const creatorData = await supabaseDb.getUser(fetchedProduct.creator_id)
          setProduct({
            ...fetchedProduct,
  ...
  useEffect(() => {
    const fetchReviews = async () => {
      ...
      try {
        const productReviews = await supabaseDb.getReviewsByProduct(product.id)

        if (productReviews.length > 0) {
          const mappedReviews = await Promise.all(productReviews.map(async r => {
            const user = await supabaseDb.getUser(r.user_id)
            return {
              id: r.id,
