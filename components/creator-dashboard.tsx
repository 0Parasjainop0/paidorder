...
export function CreatorDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const myProducts = await supabaseDb.getProductsByCreator(user.id)
      setProducts(myProducts)
      // Load Stripe Connect ID from profile (already in auth context)
  ...
  const handleSaveProduct = async () => {
    ...
    try {
      if (editingProduct) {
        const result = await supabaseDb.updateProduct(editingProduct.id, {
        ...
        const result = await supabaseDb.addProduct(productData)
        if (result) {
                    ...
                    <Button
                      onClick={async () => {
                        ...
                        try {
                          ...
                          const { error } = await updateProfile({ stripe_account_id: stripeAccountId })
                          if (error) {
