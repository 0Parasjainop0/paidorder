...
class MockDatabase {
    ...
    // --- Credentials ---
    getCredential(email: string) {
        this.data.credentials = this.data.credentials || []
        return this.data.credentials.find(c => c.email === email)
    }

    addCredential(email: string, password: string) {
        this.data.credentials = this.data.credentials || []
        // Check if credential already exists
        const existing = this.data.credentials.find(c => c.email === email)
        if (existing) {
            return false // Already registered
        }
        this.data.credentials.push({ email, password })
        this.saveData()
        return true
    }
    ...
    validateCredential(email: string, password: string): boolean {
        this.data.credentials = this.data.credentials || []
        const credential = this.data.credentials.find(c => c.email === email)
